...
jenkins_user = open("/run/secrets/jenkins-user", "r").read()
jenkins_password = open("/run/secrets/jenkins-pass", "r").read()
...
def slave_run(slave_jar, jnlp_url):
    ...
    if os.environ['SLAVE_SECRET'] == '':
        params.extend([ '-jnlpCredentials', jenkins_user + ':' + jenkins_password ])
    else:
